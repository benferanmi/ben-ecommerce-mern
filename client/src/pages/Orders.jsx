...
const Orders = () => {
    ...
    if (orderData?.length === 0) {
        return <div className="text-sm bg-slate-200 text-black flex flex-col items-center justify-centerh-[100vh] px-16 w-[100%] px-15 py-10 ">
            <p className="m-2 p-5">You Don&apos;t have An order Placed Yet. Kindly Proceed to the collection page and select some product. </p>
                        ...
                        orderData.slice(0, 5).map((item, index) => (
                            <div key={index} className="py-4 border-t border-b text-gray-700 flex flex-col md:flex-row md:items-center md:justify-between gap-4" >
                                ...
                                <div className="md:w-1/2 flex justify-between">
                                    ...
                                    <button onClick={loadOrderData} className="border px-4 py-2 text-sm font-medium text-gray-700">Track Order</button>
                                </div>
                            </div>
                        ))
                    }
